...
describe('Collapse2.vue', () => {
  ...
  test('点击标题展开/关闭内容', async () => {
    // 行为
    await firstHeader.trigger('click') // 这里click没触发， 不知道是什么原因
    console.log('firstContent', firstContent.html())
    expect(firstContent.isVisible()).toBeFalsy()
    await secondHeader.trigger('click') // // 这里click没触发， 不知道是什么原因
  ...
  test('disabled 的内容应该没有反应', async () => {
    onChange.mockClear() // 清除上面的调用次数，回归初始状态
